...
class DRBezierWaveView: UIView {
  ...
  func animation() {
    ...
    for i in 0...Int(waveWidth)-1 { // 终点和control点不用设置，通过下面的正弦函数计算，依靠addLine手动设置
      ...
      let y = Double(1.1)*Double(waveHeight)*Double(sinf(Float(a+b))) + Double((1-progress)*height)
    ...
    backg.fillColor = UIColor(red: 230 / 255.0, green: 65 / 255.0, blue: 46 / 255.0, alpha: 1).cgColor
    backg.opacity   = 0.5
